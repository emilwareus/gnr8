...
// load on a service that is already restarting. If the wait is cancelled we surface the
// transport failure rather than waitErr: cancellation stopped the retry, it is not why the
// request failed, and ctx.Err() is already available to the caller.
