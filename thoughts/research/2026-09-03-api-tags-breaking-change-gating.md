...
1. **Retain `Audience` and merely mirror it to tags.** Rejected by the owner and by rule 3. An enum
   plus standard tags would be two classification facts with a precedence problem; removing the enum
   is the point of this replacement.
2. **Add `ClassifyOperations`, `SetTags`, or `TagOperations`.** Rejected for the basic use case because
   `DocumentOperation::tag(s)` already writes the canonical tag vector
   (`crates/gnr8-sdk/src/sdk/builtins.rs:1552-1625`). A second built-in with the same output needs a
   distinction users must learn but the graph cannot observe. Extend `OperationSelector` once if
   source-prefix selection is needed.
3. **Hard-code `internal` as a reserved semantic tag.** Rejected. The owner wants different tags—such
   as `internal`, `beta`, and `partner`—selectable by policy. A reserved spelling would turn a standard,
   open string into a hidden gnr8 convention and make an innocent domain tag change the gate.
4. **Infer exemptions from `/internal`, `/_debug`, a middleware name, a file directory, or tag prefix.**
   Rejected. Each is evidence a user may select in `.gnr8/`, not a safe universal meaning. A wrong
   guess suppresses the exact breaking change this feature exists to catch, and adding an override
   produces the forbidden derive-then-fallback path.
5. **Use `Operation::group` alone.** Rejected. `group` is singular and controls native SDK service/file
   layout (`crates/gnr8-core/src/sdk/emit_common.rs:526-532`), while standard operations can carry
   several tags ([OpenAPI Operation Object](https://spec.openapis.org/oas/v3.1.2.html#operation-object)).
   The effective-tag resolver preserves the legitimate group fallback without reducing the model to it.
6. **Pre-filter the base/current documents, then run an ordinary diff.** Rejected because a tag present
   on only one side changes operation identity/existence before comparison. oasdiff documents the same
   one-sided-filter artifact for its extension filter
   ([oasdiff endpoint filtering](https://github.com/oasdiff/oasdiff/blob/main/docs/FILTERING-ENDPOINTS.md)).
   Diff first, evaluate both tag sets second.
7. **Make `--include-tag` a co-equal first-version mode.** Rejected in §4.8. It makes untagged and
   unforeseen tags non-gating, needs opposite retag logic, and creates precedence if combined with
   exemptions. The safe complement of repeatable exemptions covers the owner's immediate need.
8. **Offer both CLI flags and `.changes()` pipeline policy.** Rejected because merge/override precedence
   is a second control path. One initial surface—repeatable `--exempt-tag`—keeps the invocation auditable
   and adds no graph/protocol fact.
9. **Use root Tag Objects as the classification source.** Rejected. Root objects declare tag metadata;
   operation membership still lives on each Operation Object, and declarations are optional
   ([OpenAPI Object](https://spec.openapis.org/oas/v3.1.2.html#openapi-object),
   [Tag Object](https://spec.openapis.org/oas/v3.1.2.html#tag-object)). A root definition cannot say
   which operations carry the tag.
10. **Emit or read a vendor extension.** Rejected. Standard tags were selected specifically to avoid a
    private/foreign dialect, and repository rule 0 forbids reading another generator's convention.
    Fern and Redocly demonstrate that extension spellings are tool-specific rather than standard
    ([Fern audiences](https://buildwithfern.com/learn/api-definitions/openapi/extensions/audiences),
    [Redocly `x-tagGroups`](https://redocly.com/docs/realm/content/api-docs/openapi-extensions/x-tag-groups)).
11. **Store tags or exemption state on schemas.** Rejected. A schema shared by checked and exempt
    operations cannot have one truthful local value. Derive the most-checked consumer through existing
    graph reachability (§4.9).
12. **Delete exempt operations from the graph or generated OpenAPI.** Rejected. It makes debug/test
    endpoints unavailable to generated SDK consumers and confuses gate policy with artifact selection.
    All operations stay generated by default.

---

## 6. Open decisions

1. **How `--base <ref>` materializes the historical projected graph.** The earlier document's §3
   remains unresolved: this checkout has no git integration and no `changes` command
   (`crates/gnr8/src/cli.rs:47-96`). Whatever representation is chosen must retain each revision's
   existing `group` and `operation_docs.tags`; “read an artifact, otherwise run today's pipeline” is
   still a forbidden fallback chain.
2. **How a deliberate `C → X` narrowing is approved.** This recommendation classifies the first
   exempt-tag addition as BREAKING, so a project needs a reviewable way to land that policy change.
   The prior document's proposed content-addressed `--allow <change-id>` still fits and preserves a
   binary 0/1 gate, but issue #75 must decide that approval surface. A blanket “allow all” would undo
   the anti-retag rule.
3. **Whether effective configured tags should augment the source group.** Today a non-empty docs tag
   vector replaces the group fallback (`crates/gnr8-core/src/lower/mod.rs:760-764`). Keeping that rule
   makes the diff exactly match emitted OpenAPI. Changing it would make
   `.tag("internal")` preserve `books` automatically, but it is an existing tag-semantics change and
   should be decided/tested independently rather than smuggled into `changes`.
4. **Whether to ship synthesized root Tag Objects with the gate.** §4.10 recommends deterministic,
   name-only declarations derived from effective operation tags. They are not required by OpenAPI and
   not needed by the gate
   ([OpenAPI Object](https://spec.openapis.org/oas/v3.1.2.html#openapi-object)), so this can be sequenced
   independently. Imported descriptions, `externalDocs`, and author-controlled order need a separate
   graph design if they are ever preserved.
5. **Whether include-only demand becomes concrete.** If it does, §4.8 fixes the minimum safe shape:
   a mutually exclusive mode whose match on either extant side gates. Until then, do not weaken the
   untagged/default safe behavior for a speculative inverse.
6. **The CI trust boundary for the exempt-tag list.** The CLI applies one invocation set symmetrically
   to both historical tag sets. Editing the CI command to add a tag that was already present on both
   revisions can reinterpret those operations as exempt; removing the `changes` step could bypass the
   gate more directly. The command must echo its resolved set in human/JSON reports, and repositories
   should protect the workflow that supplies it. Technically versioning the exempt set would require
   a new historical policy fact or a separately supplied base policy—both expand beyond the owner's
   no-new-classification-protocol decision and need an explicit future decision.
7. **The explicit OpenAPI target-filter API.** The conclusion is settled—default output contains every
   operation, reduced documents are opt-in—but no tag filter exists on `OpenApi31` today. Its exact
   builder surface, schema-pruning semantics, and whether multi-tag matching is any/all remain Open.
   Redocly exposes both any/all strategies, demonstrating that the choice is real rather than implied
   by the `tags` field
   ([Redocly `filter-out`](https://redocly.com/docs/cli/decorators/filter-out)).
8. **Non-git and shallow-clone failures.** `changes` adds the repository's first git-dependent command.
   A missing ref/history, non-git checkout, or unavailable git executable must produce a typed command
   error, never an empty diff with exit `0`. The existing docs reserve non-gate nonzero statuses for
   invocation/execution failures (`docs/cli/commands.md:148-156`).

None of these open decisions changes the fixed center: standard operation tags are the classification
fact; `--exempt-tag` names the gate policy; untagged is checked; and a break is exempt only when every
extant historical side carries at least one configured exempt tag.
