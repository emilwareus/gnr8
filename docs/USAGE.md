...
`gnr8-core` and drives the lifecycle. `gnr8 init` scaffolds it; `gnr8 generate` compiles + runs it. The
...
```
.gnr8/
  Cargo.toml      # name "<dir>-gnr8-gen", edition 2021, publish=false, empty [workspace]; gnr8-core dep
  src/main.rs     # the Pipeline — THE config; you edit this
  .gitignore      # /target/  /cache/
  cache/          # ownership manifest (git-ignored)
```
The `gnr8-core` dep is a `path = "…"` dep when `.gnr8/` is inside the gnr8 repo (examples/tests), and a
version dep (`gnr8-core = "0.1"`) otherwise (publishing is a later milestone).

### The SDK surface (`gnr8_core::sdk`, re-exported as `gnr8_core::sdk::prelude`)
A pipeline composes four kinds of stage, decoupling **N sources** from **M targets** through one IR
(`gnr8_core::graph::ApiGraph`: `operations`, `schemas`, `base_path`, `title`, `security`, `diagnostics`).
...
- `gnr8_core::runner::run(pipeline) -> ExitCode` — the entry point `main()` returns. It parses argv
...
generator, no config DSL. The IR (`gnr8_core::graph`) is read/write so a `Transform` edits it freely;
